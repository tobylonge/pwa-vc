...
;(function($, window, document, undefined){
	window.app.comps.placeholderInitials = {
		config: {
			placeholderInitials: {
		...
		init:function(){
			...
			var self = this,
					cf = this.config,
					opts = cf.placeholderInitials;


					opts.$container = $('[' + self.attr_name('placeholder-initials') + ']');
					...
					opts.$container.each(function(index, elem){
						...
						var placeholderAvatar = $el.attr(self.attr_name('placeholder-initials')),
						...
						initialsContent = '<div class="user-profile-initials js-user-initials"></div>',
						$uInitialsContent;
						if(opts.useColorClasses === false){
							$uInitialsContent = $('<div class="user-profile-initials-wrapper js-user-initials-wrapper">').html(initialsContent); //Create the html
						}
						else {
							$uInitialsContent = $('<div class="user-profile-initials-wrapper js-user-initials-wrapper color-0">').html(initialsContent); //Create the html
						}
